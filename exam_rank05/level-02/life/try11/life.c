...
void	play(t_game *game)
{
	...
	for (int row = 0; row < game->height; row++)
	{
		...
		{
			int	 count = count_neighbords(game, row, col);

			if (game->board[row][col] == 'O')
			{
				new_table[row][col] = (count == 2 || count == 3) ? 'O' : ' ';
			}
			else
			{
				new_table[row][col] = (count == 3) ? 'O' : ' ';
			}
		}
	}
...
int	main(int ac, char **av)
{
	...
	while (read(STDIN_FILENO, &cmd, 1) == 1)
	{
		...
		if (game.drawing &&
			...
			game.col >= 0 && game.col < game.width && (cmd == 'w' || cmd == 's' || cmd == 'a' || cmd == 'd' || cmd == 'x'))
		{
