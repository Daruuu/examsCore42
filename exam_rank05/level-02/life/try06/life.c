...
int	count_alives(t_game *game, int row, int col)
{
	int	counter =0;
...
void play(t_game *game)
{
	...
	for(int row = 0; row < game->height; row++)
	{
		...
		{
			int	vecinos = count_alives(game, row, col);
...
int main(int ac, char **av)
{
	...
	while (read(STDIN_FILENO, &cmd, 1) == 1)
	{
		...
		if (game.drawing && game.row >= 0 && game.row < game.height && game.col >= 0 && game.col < game.width && 
